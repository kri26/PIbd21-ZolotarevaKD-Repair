...
namespace RepairBusinessLogic.BusinessLogic
{
    public class SaveToExcel
    {
                ...
                foreach (var order in dictOrders)
                {
                    ...
                    foreach (var repairWork in order.Value)
                    {
                        InsertCellInWorksheet(new ExcelCellParameters
                        {
                            ...
                            ColumnName = "B",
                            RowIndex = rowIndex,
                            Text = repairWork.RepairWorkName,
                            StyleIndex = 0U
                        });
                        InsertCellInWorksheet(new ExcelCellParameters
                        {
        ...
        public static void CreateDoc(ExcelInfoWarehouse info)
        {
            using (SpreadsheetDocument spreadsheetDocument = SpreadsheetDocument.Create(info.FileName, SpreadsheetDocumentType.Workbook))
            {
                // Создаем книгу (в ней хранятся листы)
                ...
                foreach (var warehouse in info.Warehouses)
                {
                    ...
                    foreach (var material in warehouse.Materials)
                    {
                        ...
                        InsertCellInWorksheet(new ExcelCellParameters
                        {
                            ...
                            ColumnName = "C",
                            RowIndex = rowIndex,
                            Text = material.Value.ToString(),
                            StyleIndex = 0U
                        });
                        totalPrice += material.Value;
                        rowIndex++;
                    }
                    InsertCellInWorksheet(new ExcelCellParameters
                    {
